...
- 결과 디렉터리: `results/blue2/20260518T153339+0900-m4`
...
- 광범위 ClusterRoleBinding 제거 후 잔존 개수: `0`
- 광범위 ClusterRoleBinding 제거 옵션 사용: `true`
- M4 판정: `Passed`
- 판정 이유: isolated token test denied kube-system Secrets access
...
| M4 적용 후 kube-system Secrets list 권한 | `no` |
| M4 적용 전 ingress-nginx Secrets list 권한 | `yes` |
| M4 적용 후 ingress-nginx Secrets list 권한 | `yes` |
| 그룹 포함 kube-system Secrets list 권한 | `no` |
| 그룹 포함 ingress-nginx Secrets list 권한 | `yes` |
| 격리 토큰으로 kube-system Secrets 권한 | `Forbidden` |
| 격리 토큰으로 ingress-nginx Secrets 권한 | `Allowed` |
| M4 이후 T2 토큰 접근 관찰 | `Observed` |
| M4 이후 T3 lateral movement 제한 | `Yes` |
...
- 이전 `token_test_kube_system.log`의 `Allowed` 결과는 기본 kubeconfig 사용자 인증이 섞인 비격리 테스트로 판단한다. `isolated_token_test_kube_system.log`는 빈 kubeconfig와 명시적 `--server`, `--insecure-skip-tls-verify`, `--token`만 사용했고 `authorization_decision: no`를 반환했다.
- 그룹 포함 impersonation도 kube-system Secrets list는 `no`였으므로, `system:serviceaccounts`, `system:serviceaccounts:ingress-nginx`, `system:authenticated` 그룹 바인딩이 kube-system Secrets 접근을 부여하지 않는다.
...
M4 RBAC 최소 권한화 적용 후 ingress-nginx ServiceAccount의 kube-system Secrets 접근은 `no`로 제한되었으며, 토큰 탈취 가능성은 남더라도 탈취 토큰의 lateral movement blast radius가 축소되었다.
...
| `token_test_kube_system.log` | 비격리 legacy 토큰 테스트 로그(관리자 kubeconfig 혼입 가능성 있음) |
